...
from eea.googlechartsconfig.events import GoogleChartFacetDeletedEvent
...
class Edit(BrowserView):
    ...
    def views_vocabulary(self):
        ...
        voc = queryUtility(IVocabularyFactory,
                           name=u'eea.googlechartsconfig.vocabularies.ViewsVocabulary')
...
class Configure(BrowserView):
    """ Edit controller
    """
    def _redirect(self, msg='', ajax=False, to='googlechart-edit.html'):
        """ Return or redirect
        """
        if ajax:
            return msg
    ...
    def handle_facets(self, **kwargs):
        ...
        ajax = (kwargs.get('googlechart.facets.save') == 'ajax')

        if not order:
            return self._redirect(
                'GoogleChart facets settings not saved: Nothing to do', ajax)
        ...
        return self._redirect('GoogleChart facets settings saved', ajax)
    ...
    def handle_facetDelete(self, **kwargs):
        ...
        ajax = (kwargs.get('googlechart.facet.delete') == 'ajax')
        try:
            ...
            event.notify(GoogleChartFacetDeletedEvent(self.context, facet=name))

        return self._redirect('GoogleChart facet deleted', ajax)

    def __call__(self, **kwargs):
        if self.request:
            kwargs.update(self.request.form)

        if kwargs.get('googlechart.facets.save', None):
            return self.handle_facets(**kwargs)
        elif kwargs.get('googlechart.facet.delete', None):
