...
function putImageDivInPosition(div_id, position, vhash) {
  ...
  var div =
    "<div id='" + div_id + "' class='eea-googlechart-hidden-image'></div>";
...
commonModule.insertBottomImages = function (settings, chart_url) {
  ...
  if (!is_cross_domain) {
    if (settings.iframe_qr_settings && settings.iframe_qr_settings.hide) {
      settings.qr_pos = "Disabled";
    } else {
      if (settings.iframe_qr_settings && settings.iframe_qr_settings.resize) {
    ...
    if (settings.iframe_wm_settings && settings.iframe_wm_settings.hide) {
      settings.wm_pos = "Disabled";
    } else {
      if (settings.iframe_wm_settings && settings.iframe_wm_settings.resize) {
        wm_resize = true;
      }
    }
  }

  putImageDivInPosition(
    "googlechart_qr_" + chart_hash,
    settings.qr_pos,
    chart_hash
  );

  var qr_img_url =
    "https://chart.apis.google.com/chart?cht=qr&chld=H%7C0&chs=" +
    settings.qr_size +
    "x" +
    settings.qr_size +
    "&chl=" +
    encodeURIComponent(chart_url);
  ...
  if (settings.qr_pos !== "Disabled") {
    ...
    jQuery("#googlechart_qr_" + chart_hash).removeClass(
      "eea-googlechart-hidden-image"
    );
  }

  putImageDivInPosition(
    "googlechart_wm_" + chart_hash,
    settings.wm_pos,
    settings.vhash
  );
  ...
  if (settings.wm_pos !== "Disabled") {
    ...
    jQuery("#googlechart_wm_" + chart_hash).removeClass(
      "eea-googlechart-hidden-image"
    );
  }
  if (qr_resize) {
    jQuery("#googlechart_qr_" + chart_hash + " img").css(
      "height",
      settings.iframe_qr_settings.size + "px"
    );
  }
  if (wm_resize) {
    jQuery("#googlechart_wm_" + chart_hash + " img").css(
      "height",
      settings.iframe_wm_settings.size + "px"
    );
  }
...
jQuery(function ($) {
  ...
  doc.on("googlecharts.embed.ready", function (ev, data) {
    ...
    var baseurl_uri = window.encodeURIComponent(
      embed_obj["baseurl_" + hash] + "/#tab-" + embed_obj["chart_id_" + hash]
    );
